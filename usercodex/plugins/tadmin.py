...
async def tmuter(event):  # sourcery no-metrics
    ...
    codevent = await edit_or_reply(event, "`muting....`")
    user, reason = await get_user_from_event(event, codevent)
    if not user:
        return
    if not reason:
        return await codevent.edit("you haven't mentioned time, check `.help tmute`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    codtime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ctime = await extract_time(codevent, codtime)
    if not ctime:
        return
    if user.id == event.client.uid:
        return await codevent.edit(f"Sorry, I can't mute myself")
    try:
        await codevent.client(
            EditBannedRequest(
                event.chat_id,
                user.id,
...
async def tban(event):  # sourcery no-metrics
    ...
    codevent = await edit_or_reply(event, "`banning....`")
    user, reason = await get_user_from_event(event, codevent)
    if not user:
        return
    if not reason:
        return await codevent.edit("you haven't mentioned time, check `.help tban`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    codtime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ...
    if not ctime:
        return
    if user.id == event.client.uid:
        return await codevent.edit(f"Sorry, I can't ban myself")
    await codevent.edit("`Whacking the pest!`")
    try:
        await event.client(
            EditBannedRequest(
                event.chat_id,

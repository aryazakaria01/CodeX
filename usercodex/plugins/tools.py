...
from ..helpers.utils import _codutils, _format, reply_id
...
async def _(event):
    ...
    chat = "@VS_Robot"
    codevent = await edit_or_reply(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if response1.text:
                ...
                return await codevent.edit(response1.text, parse_mode=_format.parse_pre)
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        ...
        if not input_str:
            return await edit_or_reply(codevent, response4.text)
...
async def parseqr(event):
    ...
    codevent = await edit_or_reply(event, "`Decoding....`")
    ...
    t_response, e_response = (await _codutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await edit_or_reply(codevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await edit_or_reply(codevent, f"**Failed to Decode:**\n`{result}`")
    except Exception as e:
        await edit_or_reply(codevent, f"**Error:**\n`{str(e)}`")
...
async def _(event):
    ...
    try:
        ...
        return await codevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(codevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    codevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
    ...
    if not r.ok:
        return await codevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
...
Year: {}""".format(
    ...
    await codevent.edit(output_str, link_preview=True)

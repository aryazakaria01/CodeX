...
@codex.cod_cmd(
    ...
    info={
        ...
        "description": "Reply to any external plugin(supported by codex) to install it in your bot.",
        "usage": "{tr}install",
    },
)
...
async def send(event):
    ...
    the_plugin_file = f"./usercodex/plugins/{input_str}.py"
    if os.path.exists(the_plugin_file):
        start = datetime.now()
        cood = await event.client.send_file(
            event.chat_id,
            the_plugin_file,
            force_document=True,
            allow_cache=False,

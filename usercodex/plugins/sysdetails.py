...
async def cpu(event):
    ...
    cmd = "cod /proc/cpuinfo | grep 'model name'"
    o = (await _codutils.runcmd(cmd))[0]
    await edit_or_reply(
        event, f"**[Cod](tg://need_update_for_some_feature/) CPU Model:**\n{o}"
    )
...
async def sysdetails(sysd):
    ...
    codevent = await edit_or_reply(sysd, "`Fectching system information.`")
    cmd = "git clone https://github.com/dylanaraps/neofetch.git"
    await _codutils.runcmd(cmd)
    neo = "neofetch/neofetch --off --color_blocks off --bold off --cpu_temp C \
                    --cpu_speed on --cpu_cores physical --kernel_shorthand off --stdout"
    a, b, c, d = await _codutils.runcmd(neo)
    result = str(a) + str(b)
    await edit_or_reply(codevent, "**Neofetch Result:** `" + result + "`")

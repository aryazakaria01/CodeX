...
from . import codex, edit_delete, edit_or_reply, hmention, progress, reply_id, ytsearch
...
async def kakashi(event):
    ...
    if "www.instagram.com" not in link:
        ...
        start = datetime.now()
        codevent = await edit_or_reply(event, "**Downloading.....**")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await codevent.edit("**Error:** `unblock` @instasavegrambot `and retry!`")
            return
        await codevent.delete()
        cod = await event.client.send_file(
            event.chat_id,
            video,
        )
        end = datetime.now()

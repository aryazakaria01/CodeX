...
async def q_pic(event):  # sourcery no-metrics
    ...
    codevent = await edit_or_reply(event, "__Making Quote pic....__")
...
async def stickerchat(codquotes):
    "Makes your message as sticker quote"
    reply = await codquotes.get_reply_message()
    if not reply:
        return await edit_or_reply(
            codquotes, "`I cant quote the message . reply to a message`"
        )
    fetchmsg = reply.message
    repliedreply = None
    if reply.media and reply.media.document.mime_type in ("mp4"):
        return await edit_or_reply(codquotes, "`this format is not supported now`")
    codevent = await edit_or_reply(codquotes, "`Making quote...`")
    user = (
        await codquotes.client.get_entity(reply.forward.sender)
        if reply.fwd_from
        else reply.sender
    )
    res, codmsg = await process(fetchmsg, user, codquotes.client, reply, repliedreply)
    if not res:
        return
    outfi = os.path.join("./temp", "sticker.png")
    codmsg.save(outfi)
    endfi = convert_tosticker(outfi)
    await codquotes.client.send_file(codquotes.chat_id, endfi, reply_to=reply)
    await codevent.delete()
    os.remove(endfi)
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    codevent = await edit_or_reply(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if messages_id != []:
                ...
                return await edit_delete(
                    codevent, "`I guess you have used a invalid syntax`"
                )
            response = await response
        except YouBlockedUserError:
            return await codevent.edit("```Please unblock me (@QuotLyBot) u Nigga```")

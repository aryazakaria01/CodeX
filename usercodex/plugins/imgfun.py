...
async def imirror(event):  # sourcery no-metrics
    ...
    codevent = await event.edit("__Reflecting the image....__")
    args = event.pattern_match.group(1)
    if args:
        filename = "codexuserbot.webp"
        f_format = "webp"
    else:
        filename = "codexuserbot.jpg"
        f_format = "jpeg"
    try:
...
async def irotate(event):
    ...
    args = event.pattern_match.group(1)
    codevent = await edit_or_reply(event, "__Rotating the replied media...__")
    imag = await _codtools.media_to_pic(codevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
...
async def iresize(event):
    ...
    codevent = await edit_or_reply(event, "__Resizeing the replied media...__")
    imag = await _codtools.media_to_pic(codevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
...
async def square_cmd(event):
    ...
    codevent = await event.edit("__Adding borders to make it square....__")
    try:
        imag = await _codtools.media_to_pic(codevent, reply, noedits=True)
        if imag[1] is None:
            return await edit_delete(
...
async def pic_gifcmd(event):
    ...
    if args:
        ...
        pix = 100
    codevent = await edit_or_reply(event, "__🎞Dotifying image...__")
    imag = await _codtools.media_to_pic(codevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"

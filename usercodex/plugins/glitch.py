...
from ..helpers.utils import _codtools, _codutils, reply_id
...
async def glitch(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    codinput = int(codinput) if codinput else 2
    glitch_file = await _codtools.media_to_pic(event, reply)
    ...
    if cmd:
        ...
        glitch_img = glitcher.glitch_image(img, codinput, color_offset=True)
        glitch_img.save(glitched)
        await event.client.send_file(event.chat_id, glitched, reply_to=codid)
    else:
        glitched = os.path.join("./temp", "glitched.gif")
        glitch_img = glitcher.glitch_image(img, codinput, color_offset=True, gif=True)
        DURATION = 200
        LOOP = 0
        glitch_img[0].save(
            glitched,
        ...
        xedoc = await event.client.send_file(event.chat_id, glitched, reply_to=codid)

...
async def bad(event):
    ...
    for i, each in enumerate(links, start=1):
        cod = os.path.splitext(each.text)[0]
        bg_name.append(cod)
        lbg_list += f"**{i}.**  `{cod}`\n"
    ...
    if input_str not in bg_name:
        codevent = await edit_or_reply(event, "`Give me a correct background name...`")
        await asyncio.sleep(1)
        await edit_delete(codevent, lbg_list, time=60)
    else:
...
async def pussy(event):
    ...
    if cmd == "":
        ...
        if input_str not in logo_font:
            ...
            await asyncio.sleep(1)
            await edit_delete(codevent, font_name, time=80)
        else:
            if " " in input_str:
        ...
        if input_str not in fg_name:
            codevent = await edit_or_reply(event, "`Give me a correct color name...`")
            await asyncio.sleep(1)
            await edit_delete(
                codevent,
                f"**Available color names are here:-**\n\n{fg_list}",
        ...
        cod = re.compile(r"^\-?[1-9][0-9]*\.?[0-9]*")
        isint = re.match(cod, input_str)

...
async def _(event):
    ...
    codevent = await event.get_reply_message() if event.reply_to_msg_id else event
    the_real_message = codevent.stringify()
...
async def _(event):
    ...
    codevent = await event.get_reply_message() if event.reply_to_msg_id else event
    the_real_message = _format.yaml_format(codevent)
    await edit_or_reply(event, the_real_message, parse_mode=_format.parse_pre)

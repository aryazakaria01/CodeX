...
@codex.cod_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}gs codexuserbot",
            "{tr}gs -l6 codexuserbot",
            "{tr}gs -p2 codexuserbot",
            "{tr}gs -p2 -l7 codexuserbot",
        ],
    },
)
async def gsearch(q_event):
    "Google search command."
    codevent = await edit_or_reply(q_event, "`searching........`")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        codevent = await edit_or_reply(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await codevent.edit("Found Google Result. Pouring some soup on it!")
        headers = {
        ...
        try:
            ...
            return await edit_delete(
                codevent, "`Sorry. I am unable to find similar images`"
            )
        end = datetime.now()
...
<i>fetched in {ms} seconds</i>""".format(
        ...
        codevent = event
    await edit_or_reply(codevent, OUTPUT_STR, parse_mode="HTML", link_preview=False)
...
async def _(img):
    ...
    if message and message.media:
        ...
        return
    if photo:
        codevent = await edit_or_reply(img, "`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            return await codevent.edit("`Unsupported , most likely.`")
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            ...
            return await codevent.edit("`Google told me to fuck off.`")
        os.remove(name)
        match = await ParseSauce(fetchUrl + "&preferences?hl=en&fg=1#languages")
        guess = match["best_guess"]
        imgspage = match["similar_images"]
        if guess and imgspage:
            await codevent.edit(f"[{guess}]({fetchUrl})\n\n`Looking for this Image...`")
        else:
            return await codevent.edit("`Can't find any kind similar images.`")

...
async def spam_function(event, xedoc, cod, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(cod[0])
    if len(cod) == 2:
        spam_message = str(cod[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await xedoc.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and xedoc.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            xedoc = await event.client.send_file(
                event.chat_id, xedoc, caption=xedoc.text
            )
        ...
        if BOTLOG:
            ...
            xedoc = await event.client.send_file(BOTLOG_CHATID, xedoc)
            await _codutils.unsavegif(event, xedoc)
        return
    elif event.reply_to_msg_id and xedoc.text:
        spam_message = xedoc.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
async def spammer(event):
    ...
    xedoc = await event.get_reply_message()
    ...
    await spam_function(event, xedoc, cod, sleeptimem, sleeptimet)
...
@codex.cod_cmd(
    ...
    info={
        ...
        "usage": "{tr}cspam <text>",
        "examples": "{tr}cspam Codexuserbot",
    },
)
...
@codex.cod_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using codexuserbot",
    },
)
async def tmeme(event):
...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, cod, sleeptimem, sleeptimet, DelaySpam=True)

...
from ..helpers.utils import _codtools
...
def verify_cond(codarray, text):
    return any(i in text for i in codarray)


def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"codexuserbot_{userid}_{pack}_anim"
    return f"codexuserbot_{userid}_{pack}"


def char_is_emoji(character):
    return character in codemoji.UNICODE_EMOJI["en"]
...
async def kang(args):  # sourcery no-metrics
    ...
    if not user.username:
        try:
            ...
            username = f"cod_{user.id}"
    else:
        username = user.username
    userid = user.id
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            codevent = await edit_or_reply(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await codevent.edit("check `.info stickers`")
        ...
        if (
            ...
            await codevent.edit("`Brewing a new Pack...`")
            async with args.client.conversation("Stickers") as conv:
...
async def pack_kang(event):  # sourcery no-metrics
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(codevent, "`Unsupported File!`")
            return
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("Stickers") as conv:
                    pack, codpackname = await newpacksticker(
                        codevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("Stickers") as conv:
                    pack, codpackname = await add_to_pack(
                        codevent,
                        conv,
                        event,
                        packname,
            ...
            if codpackname not in blablapacks:
                blablapacks.append(codpackname)
...
async def pic2packcmd(event):
    ...
    codevent = await edit_or_reply(event, "__🔪Cropping and adjusting the image...__")
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "CodexUserbot_" + "".join(
        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)
    )
    image = await _codtools.media_to_pic(codevent, reply, noedits=True)
    if image[1] is None:
        return await edit_delete(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    codevent = await event.edit("__Making the pack.__")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await codevent.edit("`This is not a sticker. Reply to a sticker.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
...
async def cb_sticker(event):
    ...
    codevent = await edit_or_reply(event, "`Searching sticker packs....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await edit_delete(codevent, "`No results found :(.`", 5)

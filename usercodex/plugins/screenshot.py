...
@codex.cod_cmd(
    ...
    info={
        ...
        "usage": "{tr}ss <link>",
        "examples": "{tr}ss https://github.com/Codex51/Codex",
    },
)
async def _(event):
    ...
    try:
        ...
        if not codurl:
            ...
            codurl = url(inputstr)
        if not codurl:
            return await codevent.edit("`The given input is not supported url`")
        driver.get(inputstr)
        await codevent.edit("`Calculating Page Dimensions`")
        ...
        await codevent.edit("`Stoppping Chrome Bin`")
        driver.close()
        ...
        await codevent.edit(f"`{traceback.format_exc()}`")
...
@codex.cod_cmd(
    ...
    info={
        ...
        "examples": "{tr}scapture https://github.com/Codex51/Codex",
    },
)
async def _(event):
    "To Take a screenshot of a website."
    ...
    if "image" in contentType:
        with io.BytesIO(response_api.content) as screenshot_image:
            ...
            try:
                ...
                await codevent.edit(str(e))
    else:
        await codevent.edit(f"`{response_api.text}`")

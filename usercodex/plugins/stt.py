...
async def _(event):
    ...
    codevent = await edit_or_reply(event, "`Downloading to my local, for analysis  🙇`")
    required_file_name = await event.client.download_media(reply, Config.TEMP_DIR)
    await codevent.edit("`Starting analysis, using IBM WatSon Speech To Text`")
    headers = {
    ...
    r = response.json()
    if "results" not in r:
        return await codevent.edit(r["error"])
    ...
    await codevent.edit(string_to_show)

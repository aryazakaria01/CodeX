...
async def _(event):
    ...
    try:
        ...
        if photo:
            await codevent.edit("`now, Uploading to Telegram ...`")
            if photo.endswith((".mp4", ".MP4")):
                # https://t.me/tgbetachat/324694
                size = os.stat(photo).st_size
                if size > 2097152:
                    await codevent.edit("`size must be less than 2 mb`")
                    os.remove(photo)
                    return
                codpic = None
                codvideo = await event.client.upload_file(photo)
            else:
                codpic = await event.client.upload_file(photo)
                codvideo = None
            try:
                await event.client(
                    functions.photos.UploadProfilePhotoRequest(
                        file=codpic, video=codvideo, video_start_ts=0.01
                    )
                )
            except Exception as e:
                ...
                await edit_or_reply(
                    codevent, "`My profile picture was successfully changed`"
                )
    try:
        os.remove(photo)
    except Exception as e:

...
async def codlst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        codpath = os.path.join(path, file)
        if os.path.isfile(codpath):
            codsort.append(file)
    for file in contents:
        codpath = os.path.join(path, file)
        if os.path.isdir(codpath):
            codsort.append(file)
    return codsort
...
async def upload(path, event, udir_event, codflag=None):  # sourcery no-metrics
    codflag = codflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            codpath = os.path.join(path, file)
            await upload(Path(codpath), event, udir_event)
...
async def uploadir(event):
    ...
    if os.path.isdir(path):
        ...
        UPLOAD_.uploaded = 0
        await upload(path, event, udir_event, codflag=flag)
        end = datetime.now()

...
async def paste_img(event):
    ...
    d_file_name = None
    codevent = await edit_or_reply(event, "`Pasting the text on image`")
...
async def paste_bin(event):
    ...
    codevent = await edit_or_reply(event, "`pasting text to paste bin....`")
    ...
    try:
        ...
        await codevent.edit(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await edit_delete(codevent, f"**Error while pasting text:**\n`{str(e)}`")
...
async def get_dogbin_content(event):
    ...
    codevent = await edit_or_reply(event, "`Getting Contents of pastebin.....`")
    rawurl = None
    if "raw" in url:
        rawurl = url
    if rawurl is None:
    ...
    try:
        ...
        return await codevent.edit(f"**Request timed out.**__{str(TimeoutErr)}__")
    ...
    await edit_or_reply(codevent, reply_text)
...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(codevent, f"**Error while pasting text:**\n`{str(e)}`")
    url = response["url"]
    chat = "@CorsaBot"
    await codevent.edit("`Making instant view...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await codevent.edit("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
        ...
        await codevent.edit(result, link_preview=True)

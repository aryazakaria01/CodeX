...
async def get_manga(event):
    ...
    codevent = await edit_or_reply(event, "`Searching Manga..`")
...
async def get_manga(event):
    ...
    codevent = await edit_or_reply(event, "`Searching Anime..`")
    jikan = jikanpy.jikan.Jikan()
...
async def character(event):
    ...
    codevent = await edit_or_reply(event, "`Searching Character...`")
    try:
        search_result = jikan.search("character", search_query)
    except APIException:
        return await edit_delete(codevent, "`Character not found.`")
    first_mal_id = search_result["results"][0]["mal_id"]
...
async def whatanime(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"https://api.trace.moe/search?anilistInfo&url={quote_plus(cod)}"
        ) as raw_resp0:
